...
class DecorationsManager {
    ...
    getState() {
        return {
            trees: Array.from(this.decorations.values()).filter(d => d.type === "tree").map(d => d.toJSON()),
            flowers: Array.from(this.decorations.values()).filter(d => d.type === "flower").map(d => d.toJSON()),
            benches: Array.from(this.decorations.values()).filter(d => d.type === "bench").map(d => d.toJSON()),
            lamps: Array.from(this.decorations.values()).filter(d => d.type === "lamp").map(d => d.toJSON()),
            rocks: Array.from(this.decorations.values()).filter(d => d.type === "rock").map(d => d.toJSON()),
            fountains: Array.from(this.decorations.values()).filter(d => d.type === "fountain").map(d => d.toJSON())
        };
    }
}
