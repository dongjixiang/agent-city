...
const ActionService = require('../systems/action-service');
const MessageService = require('../systems/message-service');
const MovementService = require('../systems/movement-service');
...
let movementServiceInstance = null;
let exploreManagerInstance = null;

function initServices() {
    // 广播函数
    const broadcastFn = (message) => {
        if (wsHandler) {
            wsHandler.broadcast(message);
        }
    };
    
    messageServiceInstance = new MessageService(broadcastFn);
    movementServiceInstance = new MovementService(agentStore, eventDispatcher);
    exploreManagerInstance = new ExploreManager(agentStore, null);
}

function getExploreManager() {
    return exploreManagerInstance;
}
...
class WebSocketHandler extends BaseHandler {
    ...
    async handleRegister(ws, clientId, payload) {
        ...
        if (agentStore) {
            ...
            if (!WorldData.isValidWalkPosition(safePosition.x, safePosition.z)) {
                console.log('[handleRegister] 初始位置无效，查找安全位置');
                safePosition = WorldData.findSafeSpawnPosition() || { x: 30, z: 30 };
            }
    ...
    async handleAIDecisionResponse(ws, clientId, payload) {
        ...
        // 广播智能体的行动内容到世界之窗
        if (reasoning || action) {
            const actionText = reasoning || ActionService.getDescription(action, params);
            this.broadcast({
                type: 'AGENT_THOUGHT',
                agentId,
                agentName,
        ...
        switch (action) {
            case 'move_to':
                if (params && params.x !== undefined && params.z !== undefined) {
                    ...
                    if (movementService) {
                        await movementService.handleGoTo(agentId, { x: params.x, z: params.z }, (msg) => this.broadcast(msg));
                    }
                }
                break;
            ...
            case 'rest':
                {
                    const movementService = getMovementService();
                    if (movementService) {
                        await movementService.updateState(agentId, 'resting', (msg) => this.broadcast(msg));
                    }
                }
                break;
            ...
            case 'explore':
                if (agentStore) {
                    ...
                    if (agent && agent.position) {
                        const exploreManager = getExploreManager();
                        if (exploreManager) {
                            const newPos = await exploreManager.performExplore(agentId, agent);
                            if (newPos) {
                                this.broadcast({
                                    type: 'AGENT_MOVED',
                                    agentId,
            ...
            case 'move_to':
                {
                    const movementService = getMovementService();
                    if (movementService) {
                        await movementService.handleGoTo(agentId, params, (msg) => this.broadcast(msg));
                    }
                }
                break;
            ...
            case 'stay':
                {
                    const movementService = getMovementService();
                    if (movementService) {
                        await movementService.updateState(agentId, 'idle', (msg) => this.broadcast(msg));
                    }
                }
                break;
                ...
                if (agentStore) {
                    ...
                    if (agent && agent.position) {
                        ...
                        if (exploreManager) {
                            const newPos = await exploreManager.performExplore(agentId, agent);
                            if (newPos) {
                                this.broadcast({ type: 'AGENT_MOVED', agentId, position: newPos });
                            }
                        }
                    }
                }
            ...
            case 'respond':
                if (params && params.content && params.replyTo) {
                    const agentData = agentStore ? await agentStore.get(agentId) : null;
