...
export class TankTransformer {
    constructor(scene, agentId, startPosition, entityData) {
        this.scene = scene;
        this.agentId = agentId;
        this.position = entityData?.position || startPosition || { x: 0, z: 0 };
        this.rotation = entityData?.rotation || 0;
        this.isTransformed = entityData?.isTransformed || false; // false = robot, true = tank
        this.isMoving = false;
        ...
        // 边界（来自服务器）
        this.bounds = entityData?.bounds || { minX: -1.5, maxX: 1.5, minZ: -2.5, maxZ: 2.5 };
        
        // 巡逻路线（来自服务器或默认）
        const serverPatrol = entityData?.patrolRoute;
        this.patrolRoute = serverPatrol || [
            { x: -20, z: -20 },
            { x: -15, z: -20 },
            { x: -15, z: -15 },
            { x: -20, z: -15 }
    ...
    createRobotMesh() {
        ...
        // 脚底约在 local y=-1.28，向上偏移1.5使脚正好接触地面
