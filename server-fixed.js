...
function createTeam(leaderId, leaderName) {
    ...
    AgentMemory.addAchievement(leaderId, 'team_leader');
...
function acceptInvitation(invitationId, invitedId) {
    ...
    // Add achievement
    AgentMemory.addAchievement(invitedId, 'team_player');
...
function completeTeamTask(teamId) {
    ...
    // Award all members
    team.members.forEach(member => {
        AgentMemory.addAchievement(member.id, 'task_completed');
    });
...
AgentMemory.preloadAllMemories();
AgentMemory.startAutoSave();

// Graceful shutdown - save all memories before exit
process.on('SIGINT', () => {
  console.log('\n🛑 Shutting down...');
  AgentMemory.stopAutoSave();
  process.exit(0);
});
...
function handleDialogueStart(ws, msg) {
  ...
  AgentMemory.addConversation(fromAgentId, to, targetName, content);
  AgentMemory.addConversation(to, fromAgentId, fromAgentName, content);
  
  // Check for first conversation achievement
  const fromMemory = AgentMemory.getMemory(fromAgentId);
  if (fromMemory.conversations.length === 1) {
    AgentMemory.addAchievement(fromAgentId, 'social_butterfly');
  }
...
function handleGetMemory(ws, msg) {
  ...
  const memory = AgentMemory.getMemory(targetId);
...
function handleVisitPlace(ws, msg) {
  ...
  // Record the visit
  AgentMemory.addPlace(agentId, placeName, x, z);
  
  // Check for first visit achievement
  const memory = AgentMemory.getMemory(agentId);
  if (memory.places.length === 1) {
    AgentMemory.addAchievement(agentId, 'explorer');
  }
...
function acceptFriendRequest(fromId, toId) {
    ...
    // 添加成就
    AgentMemory.addAchievement(fromId, 'made_friend');
    AgentMemory.addAchievement(toId, 'made_friend');
